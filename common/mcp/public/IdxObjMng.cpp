...
//根据内存量计算节点数量
ssize_t TIdxObjMng::CountIdxObjNum(const ssize_t MEMSIZE,ssize_t iObjSize,ssize_t iDSNum)
{
	//自己至少要用一个存储free list 指针
	int iTmpDSNum = (iDSNum < 1) ? 1 : iDSNum;
	
	ssize_t iIdxSize = sizeof(TIdx) + sizeof(idx_t)*iTmpDSNum;

	//自动计算Num
	ssize_t iLeftLen = MEMSIZE-sizeof(TIdxObjHead);
	ssize_t iIdxObjSize = iIdxSize+iObjSize;
	return (ssize_t)(iLeftLen/(float)iIdxObjSize);
}
