...
size_t CAnsyTimerQueue::GetOldestItem()
{
	ssize_t iIdx = m_stExpireQueue.GetHeadItem();
	if (iIdx < 0)
	{
		return -1;
	}
	
	THashNode*  pCurrHashNode = (THashNode*)m_stObjMng.GetAttachObj(iIdx);
	if (!pCurrHashNode)
	{
		return -2;	
	}
	
	size_t unSeq = pCurrHashNode->m_unSeq;
