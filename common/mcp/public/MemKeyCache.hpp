...
public:
	...
	enum
	{
		SUCCESS = 0,
		E_ERR_BASE	= -13200,
		E_NO_DATA = E_ERR_BASE-0,
		E_KEY_BUFF_SMALL = E_ERR_BASE-1,
		E_DATA_BUFF_SMALL = E_ERR_BASE-2,
		E_RESERVE_BUFF_SMALL = E_ERR_BASE-3,
		E_PARAM = E_ERR_BASE-4,
		E_NO_SPACE = E_ERR_BASE-5,
		E_WRITE_BINLOG = E_ERR_BASE-6,
		E_DATA_CRASH = E_ERR_BASE-7
	};
	...
	//初始
	static ssize_t CountBaseSize(ssize_t iBucketNum,ssize_t iHashNum,ssize_t iBlockSize=512);
	static ssize_t CalcNodeNum(const ssize_t MEMSIZE,ssize_t iBlockSize,double fBlockDivNode=1.1);
	static ssize_t CalcObjNum(const ssize_t MEMSIZE,ssize_t iNodeNum,ssize_t iBlockSize);
	...
	*/
	ssize_t AttachMem(char* pMemPtr,const ssize_t MEMSIZE,ssize_t iBucketNum,ssize_t iNodeNum,ssize_t iInitType,ssize_t iBlockSize);
	ssize_t AttachMem(char* pMemPtr,const ssize_t MEMSIZE,ssize_t iNodeNum=0,ssize_t iInitType=emInit,ssize_t iBlockSize=512);
	...
	*/
	ssize_t DumpInit(ssize_t iDumpMin=24*60,char *DumpFile="cache.dump",
				...
				ssize_t iBinLogOpen=1,char * pBinLogBaseName="binlog",
				ssize_t iMaxBinLogSize=30000000,
				ssize_t iMaxBinLogNum=10,ssize_t iBinlogCommitSecs=2);
	...
	void IgnoreInvalidIdx(ssize_t iNodeIdx);
	//节点遍历操作
	...
	ssize_t GetDataSize(const char* pKey,const ssize_t KEYSIZE);
	...
	ssize_t GetOldestNodeIdx(ssize_t iLRUType=em_r_list);
